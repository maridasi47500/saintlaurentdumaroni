...
var url = 'https://c.tmyzer.com/c/?s=90615&f=30&fi=99';
...
delete Array.prototype.removeElement;
            ...
            return false;
                ...
                var hb_adid = window.targetingParams[formatid].hb_adid;
                            ...
                            //console.log(hb_adid);
                            if(tmzr.getHighestCpmBids('35757')[0].width<=window.inImageTmzr.lastBid.width&&tmzr.getHighestCpmBids('35757')[0].height<=window.inImageTmzr.lastBid.height){
            ...
            var refreshVisibility35757 = function(){
                        var observers = [];
            ...
            //window.adsArea35757= el;
            window.adsArea35757.isVisible = false;
            window.adsArea35757.refreshTimer = Date.now();
            window.adsArea35757.formatId = adsAreaId;
            window.adsArea35757.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea35757.capping = 1;
            window.adsArea35757.cappingLimit = adsCappingLimit;
            ...
            if (Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea35757.isVisible) {
            window.adsArea35757.isVisible = true;
            window.adsArea35757.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea35757.isVisible) {
            window.adsArea35757.isVisible = false;
            window.adsArea35757.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers35757 = new IntersectionObserver(callback, option, true, 1000);

                            observers35757.observe(document.querySelector("#sas_" + adsAreaId));
                        } else {
            window.adsArea35757.lastPos = 0;
            window.adsArea35757.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea35757.lastPos - window.scrollY) > 100) || (Date.now() - window.adsArea35757.lastTime > 200)) {
            window.adsArea35757.lastPos = window.scrollY;
            window.adsArea35757.lastTime = Date.now();
            var visibilitySensor = true;
            el = document.querySelector('#sas_35757')
            ...
            if (visibilitySensor && !window.adsArea35757.isVisible) {
            window.adsArea35757.isVisible = true;
            window.adsArea35757.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (!visibilitySensor && window.adsArea35757.isVisible) {
            window.adsArea35757.isVisible = false;
            window.adsArea35757.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            }
            ...
            refreshQueueManager35757 = function (formatId, capping) {
            window.adsArea35757.capping++;
            window.adsArea35757.refreshTimer = Date.now();

            var isClosed = false;
            if(window.adsArea35757.isClosed != undefined){
                if(window.adsArea35757.isClosed){
                    isClosed = true
                }
            }
            ...
            if(formatId == 26328){
                if(isClosed == false){
                    refreshSlotFooter(35757, window.adsArea35757.capping);
                }
            }else{
            refreshSlot(35757, window.adsArea35757.capping,window.adsArea35757.isVisible);
            }
            }
            setInterval(function(){loopChecker35757();}, 1000);
            loopChecker35757 = function() {
            if (typeof tmzr.getAllWinningBids == 'undefined') {return}
                el = window.adsArea35757;
                if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
                    lastBidder35757 = {"bidderCode":undefined, "responseTimestamp":0};
                    tmzr.getAllWinningBids().forEach(function(bid){
                        if(bid.adUnitCode==el.formatId){
                            if(bid.responseTimestamp > lastBidder35757.responseTimestamp){
                                lastBidder35757 = bid;
                            }
                        }
                    })
                    window.adsArea35757.lastBidder = lastBidder35757;
                    if(slowBidders.indexOf(el.lastBidder.bidder)==-1){
                        slowdown = 0;
                    } else {
                        slowdown = parseInt(slowBiddersTimer[slowBidders.indexOf(el.lastBidder.bidder)]);
                    }
                    if ((window.adsArea35757.refreshTimer + el.refreshRate) < Date.now()) {
                        if(((el.refreshTimer + slowdown) < Date.now()) && slowBidders.includes(el.lastBidder.bidder) == true) {
                            refreshQueueManager35757(el.formatId, el.capping);
                    ...
                    if (window.adsArea35757.refreshTimer + 25000 < Date.now() && !slowBidders.includes(el.lastBidder.bidder)) {
                        refreshQueueManager35757(el.formatId, el.capping);
                    } else if ((window.adsArea35757.refreshTimer + slowdown < Date.now() && slowBidders.includes(el.lastBidder.bidder))) {
                        refreshQueueManager35757(el.formatId, el.capping);
                    }
                    ...
                    if (window.adsArea35757.refreshTimer + el.refreshRate < Date.now()) {
                        refreshQueueManager35757(el.formatId, el.capping);
                            ...
                            window.inImageSiteId = 90615;
                var inimagejs = document.createElement("script");
                inimagejs.setAttribute("type", "text/javascript");
                inimagejs.setAttribute("src","https://ads.themoneytizer.com/moneyinimage.js");
                inimagejs.setAttribute("async", "true");
                var headelement = document.getElementsByTagName('head')[0];
                headelement.appendChild(inimagejs);
                if(typeof crtg_content === 'undefined'){var crtg_content = "";}
                var mydiv = document.getElementById("90615-30");
                var creatediv = document.createElement("div");
                ...
                refreshVisibility35757();

                window.Adcall_35757= function(){
                window.targetingParams = window.targetingParams || [];
                var rtbok = "";
                window.targetingParams[35757] = window.targetingParams[35757] || 0;
                if(window.targetingParams[35757] == 0){
                var moneybid = "";
                }else{
                var hb_adid = "hb_adid="+window.targetingParams[35757].hb_adid;
                //console.log(window.targetingParams[35757]);
                //console.log(hb_adid);
                var hb_pb = "hb_pb="+window.targetingParams[35757].hb_bid;
                var hb_bidder = "hb_bidder="+window.targetingParams[35757].hb_bidder;
                if(window.targetingParams['35757'].hb_bidder == "improvedigital" || window.targetingParams['35757'].hb_bidder == "appnexus"|| window.targetingParams['35757'].hb_bidder == "oftmedia" || window.targetingParams['35757'].hb_bidder == "onetag" || window.targetingParams['35757'].hb_bidder == "rubicon" || window.targetingParams['35757'].hb_bidder == "openx" || window.targetingParams['35757'].hb_bidder == "ix" || window.targetingParams['35757'].hb_bidder == "smilewanted"){
                var hb_format = "hb_format=35757";
                }else{
                var hb_format = "hb_format="+35757;
                }
                var moneybid = ";"+hb_adid+";"+hb_pb+";"+hb_bidder+";"+hb_format+";"+rtbok;
                }
                sc.text = sas.cmd.push(function(){sas.call('std',{siteId: 493709, pageId: 1541741, formatId: 35757, target: crtg_content+moneybid});});
                creatediv.appendChild(sc);
                var http = new XMLHttpRequest();
                var url = 'https://c.tmyzer.com/c/?s=90615&f=30&fi=0';
                http.open("GET", url, true);
                http.send();
                }
