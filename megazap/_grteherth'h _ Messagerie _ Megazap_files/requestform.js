...
var url = 'https://c.tmyzer.com/c/?s=90615&f=5&fi=99';
...
delete Array.prototype.removeElement;
            ...
            var refreshVisibility26325 = function(){
                        var observers = [];
            ...
            //window.adsArea26325= el;
            window.adsArea26325.isVisible = false;
            window.adsArea26325.refreshTimer = Date.now();
            window.adsArea26325.formatId = adsAreaId;
            window.adsArea26325.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea26325.capping = 1;
            window.adsArea26325.cappingLimit = adsCappingLimit;
            ...
            if (Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea26325.isVisible) {
            window.adsArea26325.isVisible = true;
            window.adsArea26325.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea26325.isVisible) {
            window.adsArea26325.isVisible = false;
            window.adsArea26325.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers26325 = new IntersectionObserver(callback, option, true, 1000);

                            observers26325.observe(document.querySelector("#sas_" + adsAreaId));
                        } else {
            window.adsArea26325.lastPos = 0;
            window.adsArea26325.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea26325.lastPos - window.scrollY) > 100) || (Date.now() - window.adsArea26325.lastTime > 200)) {
            window.adsArea26325.lastPos = window.scrollY;
            window.adsArea26325.lastTime = Date.now();
            var visibilitySensor = true;
            el = document.querySelector('#sas_26325')
            ...
            if (visibilitySensor && !window.adsArea26325.isVisible) {
            window.adsArea26325.isVisible = true;
            window.adsArea26325.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if (!visibilitySensor && window.adsArea26325.isVisible) {
            window.adsArea26325.isVisible = false;
            window.adsArea26325.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            }
            ...
            refreshQueueManager26325 = function (formatId, capping) {
            window.adsArea26325.capping++;
            window.adsArea26325.refreshTimer = Date.now();

            var isClosed = false;
            if(window.adsArea26325.isClosed != undefined){
                if(window.adsArea26325.isClosed){
                    isClosed = true
                }
            }
            ...
            if(formatId == 26328){
                if(isClosed == false){
                    refreshSlotFooter(26325, window.adsArea26325.capping);
                }
            }else{
            refreshSlot(26325, window.adsArea26325.capping,window.adsArea26325.isVisible);
            }
            }
            setInterval(function(){loopChecker26325();}, 1000);
            loopChecker26325 = function() {
            if (typeof tmzr.getAllWinningBids == 'undefined') {return}
                el = window.adsArea26325;
                if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
                    lastBidder26325 = {"bidderCode":undefined, "responseTimestamp":0};
                    tmzr.getAllWinningBids().forEach(function(bid){
                        if(bid.adUnitCode==el.formatId){
                            if(bid.responseTimestamp > lastBidder26325.responseTimestamp){
                                lastBidder26325 = bid;
                            }
                        }
                    })
                    window.adsArea26325.lastBidder = lastBidder26325;
                    if(slowBidders.indexOf(el.lastBidder.bidder)==-1){
                        slowdown = 0;
                    } else {
                        slowdown = parseInt(slowBiddersTimer[slowBidders.indexOf(el.lastBidder.bidder)]);
                    }
                    if ((window.adsArea26325.refreshTimer + el.refreshRate) < Date.now()) {
                        if(((el.refreshTimer + slowdown) < Date.now()) && slowBidders.includes(el.lastBidder.bidder) == true) {
                            refreshQueueManager26325(el.formatId, el.capping);
                    ...
                    if (window.adsArea26325.refreshTimer + 25000 < Date.now() && !slowBidders.includes(el.lastBidder.bidder)) {
                        refreshQueueManager26325(el.formatId, el.capping);
                    } else if ((window.adsArea26325.refreshTimer + slowdown < Date.now() && slowBidders.includes(el.lastBidder.bidder))) {
                        refreshQueueManager26325(el.formatId, el.capping);
                    }
                    ...
                    if (window.adsArea26325.refreshTimer + el.refreshRate < Date.now()) {
                        refreshQueueManager26325(el.formatId, el.capping);
                                ...
                                window.TMSkin_CustomCode = ``;
                    window.TMSkin_CustomVar= `compatible=true`;
                    var skinjs = document.createElement("script");
                    skinjs.setAttribute("type", "text/javascript");
                                            skinjs.setAttribute("src","https://ads.themoneytizer.com/moneyskin.js");
                                        skinjs.setAttribute("async", "true");
                    var headelement = document.getElementsByTagName('head')[0];
                    headelement.appendChild(skinjs);
                                if(typeof crtg_content === 'undefined'){var crtg_content = "";}
                var mydiv = document.getElementById("90615-5");
                var creatediv = document.createElement("div");
                creatediv.id = "sas_26325";
                mydiv.appendChild(creatediv);
                var sc = document.createElement("script");
                sc.type = "text/javascript";
                sc.async = true;
                window.Adcall_26325= function(){
                window.targetingParams = window.targetingParams || [];
                                    var rtbok = "maxime_script=1";
                                window.targetingParams[26325] = window.targetingParams[26325] || 0;
                if(window.targetingParams[26325] == 0){
                var moneybid = "";
                }else{
                var hb_adid = "hb_adid="+window.targetingParams[26325].hb_adid;
                var hb_pb = "hb_pb="+window.targetingParams[26325].hb_bid;
                var hb_bidder = "hb_bidder="+window.targetingParams[26325].hb_bidder;
                if(window.targetingParams['26325'].hb_bidder == "improvedigital" || window.targetingParams['26325'].hb_bidder == "appnexus"|| window.targetingParams['26325'].hb_bidder == "oftmedia" || window.targetingParams['26325'].hb_bidder == "onetag" || window.targetingParams['26325'].hb_bidder == "rubicon" || window.targetingParams['26325'].hb_bidder == "openx" || window.targetingParams['26325'].hb_bidder == "ix" || window.targetingParams['26325'].hb_bidder == "smilewanted"){
                var hb_format = "hb_format=26325-1";
                }else{
                var hb_format = "hb_format="+26325;
                }
                var moneybid = ";"+hb_adid+";"+hb_pb+";"+hb_bidder+";"+hb_format+";"+rtbok;
                }
                sc.text = sas.cmd.push(function(){sas.call('std',{siteId: 493709, pageId: 1541741, formatId: 26325, target: crtg_content+moneybid});});
                creatediv.appendChild(sc);
                var http = new XMLHttpRequest();
                var url = 'https://c.tmyzer.com/c/?s=90615&f=5&fi=0';
                http.open("GET", url, true);
                http.send();
                }
